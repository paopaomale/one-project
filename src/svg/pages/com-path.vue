<template>
  <div class="page" >
    ...
    <scroller class="main">
    <refresh class="refresh"  @pullingdown="onpullingdown" :display="refreshing ? 'show' : 'hide'">
        <svg class="item-shape">
          <path d="M28,1 A27,27 0 1,1 21.01188578223194,1.9200026901951546" stroke="red" stroke-width="1" fill="transparent" :stroke-dasharray="[dasharray,1000]"/>  
        </svg>
    </refresh>
...
<script>
  ...
  module.exports = {
    ...
    methods: {
      onpullingdown (event) {
        if(event.pullingDistance<=0){
          this.dasharray = -event.pullingDistance/3+30;
        }
      }
    }
